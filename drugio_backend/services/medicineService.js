...
export const getAllMedicines = async () => {
  return query(
    ...
    SELECT 
      ...
      dist.distributor_Name AS localDistributorName,
      dist.contact_Number AS localDistributorContact,
      dist.address AS localDistributorAddress
    FROM medicine m
...
export const addMedicine = async (medicineData) => {
  ...
  try {
    ...
    let generic = await query(
      `SELECT generic_Id FROM generic WHERE genericName = ?`,
      [genericName]
    );
    let generic_Id;
    if (generic.length > 0) {
      ...
      const genericInsert = await query(
    ...
    if (brand.length > 0) {
      ...
      const brandInsert = await query(
        `INSERT INTO brand (brandName, generic_Id) VALUES (?, ?)`,
        [brandName, generic_Id]
      );
...
export const deleteMedicine = async (medicineId) => {
  return query(`DELETE FROM medicine WHERE medicine_Id = ?`, [medicineId]);
};
